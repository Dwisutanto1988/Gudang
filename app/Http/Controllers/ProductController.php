...
class ProductController extends Controller
{
    ...
    public function products(Request $req)
    {
        ...
        foreach ($products as $p) {
            $totalStockIn = DB::table('stock')
                ->where([['product_id', $p->product_id], ['type', 1]])
                ->sum('product_amount');
            $totalStockOut = DB::table('stock')
                ->where([['product_id', $p->product_id], ['type', 0]])
                ->sum('product_amount');
    ...
    public function products_wip(Request $req)
    {
        ...
        if (empty($sort)) {
            $products = $products
                ->orderBy('products_wip.product_wip_id', 'desc')
                ->paginate(50);
        } elseif ($sort == 'desc') {
            $products = $products
                ->orderBy('products.product_code', 'desc')
    ...
    public function product_save(Request $req)
    {
        $req->validate(
            [
                'product_code' =>
                    'required|unique:products,product_code,' .
                    $req->id .
                    ',product_id|numeric',
    ...
    public function product_wip_save(Request $req)
    {
        $req->validate(
            [
                'product_code' =>
                    'required|exists:products,product_code|numeric',
            ...
            [
                ...
                'product_amount.numeric' =>
                    'Product Amount harus berupa angka!',
            ]
        );
        ...
        if (empty($req->id)) {
            ...
            $update = DB::table('products_wip')
                ->where('product_wip_id', $req->id)
                ->update($data);
    ...
    public function product_wip_complete(Request $req)
    {
        ...
        $wip = DB::table('products_wip')
            ->select('*')
            ->where('product_wip_id', $wip_id)
            ->first();
        ...
        if ($wipComplete) {
            ...
            $req
                ...
                ->flash(
                    'success',
                    'Product telah dipindahkan ke Products List.'
                );
        } else {
            $req
                ->session()
                ->flash('error', 'Terjadi kesalahan! Mohon coba kembali!');
        }
    ...
    public function product_stock(Request $req)
    {
        ...
        if (!empty($amount)) {
            if (!empty($req->shelf)) {
                ...
                $endingTotalStockIn = DB::table('stock')
                    ->where([['product_id', $product_id], ['type', 1]])
                    ->sum('product_amount');
                $endingTotalStockOut = DB::table('stock')
                ...
                if ($type == 0) {
                    if ($amount > $availableStock) {
                        $result = [
                            'status' => 0,
                            'message' =>
                                'Jumlah stock out melebihi jumlah stock yang tersedia di shelf yang dipilih!',
                        ];
                        goto resp;
                    } else {
                ...
                if ($updateStock) {
                    ...
                    $result = [
                        'status' => 0,
                        'message' => 'Stok gagal diupdate! Mohon coba kembali!',
                    ];
                }
            } else {
    ...
    public function product_stock_history(Request $req)
    {
        ...
        $history = $history->paginate(100000000);
    ...
    public function categories_save(Request $req)
    {
        ...
        if (empty($category_id)) {
            ...
            if ($add) {
                ...
                $req
                    ->session()
                    ->flash('error', 'Kategori baru gagal ditambahkan!');
            }
        } else {
    ...
    public function categories_delete(Request $req)
    {
        $del = DB::table('categories')
            ->where('category_id', $req->delete_id)
            ->delete();
    ...
    public function shelf(Request $req)
    {
        ...
        if ($req->format == 'json') {
            if (!empty($product_id)) {
                $shelf = $shelf
                    ->join('stock', 'shelf.shelf_id', 'stock.shelf_id')
                    ->where('stock.product_id', $product_id)
                    ->groupBy('shelf_id');
                $result = [];
                $shelf = $shelf
                    ->select('shelf.*', 'stock.product_amount')
                    ->get();
                foreach ($shelf as $s) {
                    $totalStockIn = DB::table('stock')
                        ->where([
    ...
    public function tujuh(Request $req)
    {
        $tujuan = DB::table('tujuans');
        $result1 = $tujuan->select('tujuan', 'id')->get();
    ...
    public function shelf_save(Request $req)
    {
        ...
        if (empty($shelf_id)) {
            ...
            if ($add) {
                $req
                    ->session()
                    ->flash('success', 'Shelf baru berhasil ditambahkan.');
            } else {
                $req
                    ->session()
                    ->flash('error', 'Shelf baru gagal ditambahkan!');
            }
        } else {
            $edit = DB::table('shelf')
    ...
    public function generateBarcode(Request $req)
    {
        $code = $req->code;
        $print = $req->print;
        $barcodeB64 = DNS1D::getBarcodePNG(
            '' . $code . '',
            'C128',
            2,
            81,
